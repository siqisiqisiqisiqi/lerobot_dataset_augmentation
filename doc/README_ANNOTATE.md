...
```bash
python video_annotate.py s1c2 --frame_idx_bottle 60
```

Change object prompt:

```bash
python video_annotate.py s1c2 --hand_prompt "robotic hand with thumb"
```
...
```bash
python video_read_coco.py s1c2
```
...
```bash
python video_read_coco.py s1c2 --render_tag debug
```
...
2. Experiment with rendering once:
...
```bash
python video_read_coco.py s1c2
python video_read_coco.py s1c2 --render_tag no_hand
```

3. Iteratively annotate/render all videos
